"""System prompts for RLM - Paper-aligned (Appendix D)

Source: /tmp/arxiv-2512/appendix/sec3-methods.tex
"""

# (2) RLM with REPL (no sub-calls) - Lines 89-142 in sec3-methods.tex
RLM_SYSTEM_PROMPT = """You are tasked with answering a query with associated context. You can access, transform, and analyze this context interactively in a REPL environment, which you are strongly encouraged to use as much as possible. You will be queried iteratively until you provide a final answer.
...
# (1a) RLM with REPL (sub-calls) - Lines 5-77 in sec3-methods.tex
...
# (1b) Qwen diff - Lines 79-86 in sec3-methods.tex
# Added after line 15 (after "Use these variables as buffers...")
QWEN_SUBCALL_WARNING = """
...
def get_system_prompt(
    ...
    model_name: str = "",
    with_subcalls: bool = False
) -> str:
    """
    Get the formatted system prompt (paper-aligned).

    Args:
        context_type: Type of context (e.g., "string", "List[str]")
        context_total_length: Total character count of context
        context_lengths: List of chunk lengths (e.g., [len(context)] for string)
        model_name: Model name (for Qwen-specific additions)
    ...
    # Format context_lengths as string
    ...
    # Add Qwen warning for sub-calls prompt only
    if with_subcalls and "qwen" in model_name.lower():
        # Insert after "Use these variables as buffers to build up your final answer."
        insert_marker = "Use these variables as buffers to build up your final answer."
        if insert_marker in prompt:
            prompt = prompt.replace(
                insert_marker,
                insert_marker + QWEN_SUBCALL_WARNING
            )
